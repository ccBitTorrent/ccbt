...
class SocketOptimizer:
    ...
    def optimize_socket(self, sock: socket.socket, socket_type: SocketType) -> None:
        ...
        if not config:
            self.logger.warning(f"No configuration found for socket type: {socket_type}")
            return
        ...
        try:
            ...
            if config.so_keepalive:
                ...
                try:
                    ...
                    sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPCNT, config.tcp_keepalive_probes)
            ...
            self.logger.debug(f"Optimized socket for {socket_type}")

        except OSError as e:
            self.logger.warning(f"Failed to optimize socket: {e}")
            raise NetworkError(f"Socket optimization failed: {e}")

    def create_optimized_socket(self, socket_type: SocketType,
                               family: int = socket.AF_INET,
                               sock_type: int = socket.SOCK_STREAM) -> socket.socket:
...
class ConnectionPool:
    ...
    def __init__(self, max_connections: int = 100,
        ...
        # Start cleanup task
        self._cleanup_task = threading.Thread(target=self._cleanup_connections, daemon=True)
        self._cleanup_task.start()

    def get_connection(self, host: str, port: int,
                      socket_type: SocketType = SocketType.PEER_CONNECTION) -> Optional[socket.socket]:
        ...
        with self.lock:
            if self.connections.get(key):
                ...
                self.logger.debug(f"Reused connection to {host}:{port}")
                return sock
            ...
            try:
                ...
                self.logger.warning(f"Failed to create connection to {host}:{port}: {e}")
                return None
    ...
    def return_connection(self, sock: socket.socket, host: str, port: int) -> None:
        ...
        with self.lock:
            ...
            if len(self.connections[key]) < self.max_connections:
                ...
                self.stats.active_connections -= 1
                self.logger.debug(f"Returned connection to {host}:{port}")
            else:
                # Pool is full, close connection
                self._remove_connection(sock)

    def _create_connection(self, host: str, port: int, socket_type: SocketType) -> Optional[socket.socket]:
                ...
                with self.lock:
                    ...
                    for key, connections in self.connections.items():
                        for sock in connections[:]:
                            if (sock in self.last_activity and
                                current_time - self.last_activity[sock] > self.idle_timeout):
...
class NetworkOptimizer:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "socket_configs": {t.value: c for t, c in self.socket_optimizer.configs.items()},
        }
