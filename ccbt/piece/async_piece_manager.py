...
class AsyncPieceManager:
    ...
    def __init__(
        ...
        self._piece_selection_metrics: dict[str, Any] = {
            ...
            "no_progress_gate_request_timeouts": 0,  # No-progress gate due to timeout-stalled selector state
    ...
    def _assess_no_progress_peer_readiness(
        ...
        """Assess whether active peers have piece availability and requestability."""
        has_piece_info = False
        requestable_piece_peers = 0
        choked_piece_peers = 0
        ...
        for peer in active_peers:
            ...
            if can_request:
                requestable_piece_peers += 1
            else:
                choked_piece_peers += 1

        return has_piece_info, requestable_piece_peers, choked_piece_peers
    ...
    async def _get_peers_for_piece(
        ...
        if len(peers_to_use) <= 3 and peers_to_use:
            ...
            recent_peer_scores = {
                peer: peer_recent_unchoke_score(peer) for peer in peers_to_use
            }
            ...
            def sparse_request_peer_score(
                ...
                return (
                    recent_peer_scores.get(peer, 0.0),
        ...
        while not self._stopping:  # pragma: no cover - Infinite background loop, cancellation tested via selector_cancellation test
            try:
                ...
                if self._no_progress_stall_until > time.time():
                    ...
                    await asyncio.sleep(min(base_interval, stall_remaining))
                    self._piece_selection_metrics["selection_no_progress_streak"] = (
                        self._no_progress_streak
                    )
                    self.logger.debug(
                        "Piece selector stalled by no-progress gate for %.2fs",
                        stall_remaining,
                    )
                    continue
                ...
                if new_active_downloads > active_downloads:
                    ...
                    if (
                        ...
                        if active_peer_count_for_cycle == 0:
                            ...
                            no_progress_reason = "request_timeouts"
                        ...
                        self.logger.warning(
                            "⚠️ PIECE_SELECTOR: No-progress gate engaged after %s consecutive stalls due to %s; "
                            "pausing selection for %.2fs (active_peers=%d, requestable_piece_peers=%d, choked_piece_peers=%d)",
    ...
    async def _select_rarest_first(self) -> None:
        ...
        async with self.lock:
            ...
            if selected_pieces:
                look_ahead_count = min(
                    adaptive_request_count, len(piece_scores) - len(selected_pieces)
                )
                if (
                    look_ahead_count > 0
                    and len(selected_pieces) < adaptive_request_count
                ):
                    # Select additional pieces for look-ahead (will be requested in next cycle)
                    for _score, piece_idx in piece_scores[
                        len(selected_pieces) : len(selected_pieces) + look_ahead_count
                    ]:
                        if self.pieces[piece_idx].state == PieceState.MISSING:
