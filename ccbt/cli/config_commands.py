...
@config.command("show")
@click.option("--format", "format_", type=click.Choice(["toml", "json", "yaml"]), default="toml")
@click.option("--section", type=str, default=None, help="Show specific section key path (e.g. network)")
@click.option("--key", type=str, default=None, help="Show specific key path (e.g. network.listen_port)")
@click.option("--config", "config_file", type=click.Path(exists=True), default=None)
def show_config(format_: str, section: Optional[str], key: Optional[str], config_file: Optional[str]):
    ...
    if format_ == "json":
        ...
        try:
            ...
            raise click.ClickException("PyYAML is required for YAML output")
...
@click.option("--global", "global_flag", is_flag=True, help="Set value in global config file")
@click.option("--local", "local_flag", is_flag=True, help="Set value in project local ccbt.toml")
@click.option("--config", "config_file", type=click.Path(), default=None)
def set_value(key: str, value: str, global_flag: bool, local_flag: bool, config_file: Optional[str]):
...
@click.option("--config", "config_file", type=click.Path(), default=None)
def reset_config(section: Optional[str], key: Optional[str], confirm: bool, config_file: Optional[str]):
    """Reset configuration to defaults (optionally for a section/key)."""
    if not confirm:
        raise click.ClickException("Use --confirm to proceed with reset")
    cm = ConfigManager(config_file)
    data = cm.config.model_dump(mode="json")
...
@click.option("--config", "config_file", type=click.Path(exists=True), default=None)
def migrate_config_cmd(from_version: Optional[str], to_version: Optional[str], backup: bool, config_file: Optional[str]):
