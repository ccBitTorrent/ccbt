...
class TorrentMetrics:
    ...
    piece_availability_distribution: dict[int, int] = field(
        default_factory=dict
    )  # availability_count -> number_of_pieces
    ...
    peer_performance_distribution: dict[str, int] = field(
        default_factory=dict
    )  # performance_tier -> count
    peer_download_speeds: list[float] = field(
        default_factory=list
    )  # List of download speeds per peer
    ...
    pieces_per_second_history: list[float] = field(
        default_factory=list
    )  # Historical completion rates

    # Swarm efficiency
    swarm_efficiency: float = 0.0  # Overall swarm efficiency (0.0-1.0)
    peer_contribution_balance: float = (
        0.0  # How balanced peer contributions are (0.0-1.0)
    )
...
class MetricsCollector:
    ...
    async def _calculate_global_rates(self) -> None:
        ...
        for torrent_metrics in self.torrent_metrics.values():
            if (
                hasattr(torrent_metrics, "download_rate")
                and torrent_metrics.download_rate > 0
            ):
                total_torrent_download += torrent_metrics.download_rate
            if (
                hasattr(torrent_metrics, "upload_rate")
                and torrent_metrics.upload_rate > 0
            ):
        ...
        if torrent_count > 0:
            ...
            avg_torrent_download = (
                total_torrent_download / torrent_count if torrent_count > 0 else 0.0
            )
            avg_torrent_upload = (
                total_torrent_upload / torrent_count if torrent_count > 0 else 0.0
            )
    ...
    def update_torrent_status(self, torrent_id: str, status: dict[str, Any]) -> None:
        ...
        if "piece_availability" in status:
            ...
            if availability_list:
                ...
                metrics.average_piece_availability = (
                    sum(availability_list) / len(availability_list)
                    if availability_list
                    else 0.0
                )
                metrics.rarest_piece_availability = (
                    min(availability_list) if availability_list else 0
                )
        ...
        if "peer_download_speeds" in status:
            ...
            if metrics.peer_download_speeds:
                metrics.average_peer_download_speed = sum(
                    metrics.peer_download_speeds
                ) / len(metrics.peer_download_speeds)
                sorted_speeds = sorted(metrics.peer_download_speeds)
                metrics.median_peer_download_speed = (
                    sorted_speeds[len(sorted_speeds) // 2] if sorted_speeds else 0.0
                )
                metrics.fastest_peer_speed = (
                    max(metrics.peer_download_speeds)
                    if metrics.peer_download_speeds
                    else 0.0
                )
                metrics.slowest_peer_speed = (
                    min(metrics.peer_download_speeds)
                    if metrics.peer_download_speeds
                    else 0.0
                )
            ...
            if elapsed_time > 0:
                ...
                if metrics.piece_completion_rate > 0:
                    metrics.estimated_time_remaining = (
                        remaining_pieces / metrics.piece_completion_rate
                    )
                else:
        ...
        if metrics.active_peers > 0 and metrics.download_rate > 0:
            ...
            theoretical_max = (
                metrics.active_peers * metrics.average_peer_download_speed
                if metrics.average_peer_download_speed > 0
                else metrics.download_rate
            )
            if theoretical_max > 0:
                metrics.swarm_efficiency = min(
                    1.0, metrics.download_rate / theoretical_max
                )
            else:
            ...
            if len(metrics.peer_download_speeds) > 1:
                ...
                metrics.peer_contribution_balance = (
                    1.0  # Single peer = perfectly balanced
                )
    ...
    def update_peer_metrics(self, peer_key: str, metrics_data: dict[str, Any]) -> None:
        ...
        if "piece_download_speeds" in metrics_data:
            metrics.piece_download_speeds.update(
                metrics_data.get("piece_download_speeds", {})
            )
        if "piece_download_times" in metrics_data:
            metrics.piece_download_times.update(
                metrics_data.get("piece_download_times", {})
            )

        # Calculate pieces per second
        if metrics.connection_duration > 0 and metrics.pieces_received > 0:
            metrics.pieces_per_second = (
                metrics.pieces_received / metrics.connection_duration
            )
        ...
        if "bandwidth_utilization" in metrics_data:
            metrics.bandwidth_utilization = metrics_data.get(
                "bandwidth_utilization", 0.0
            )

        # Connection quality metrics
        if "connection_quality_score" in metrics_data:
            metrics.connection_quality_score = metrics_data.get(
        ...
        if "average_block_latency" in metrics_data:
            metrics.average_block_latency = metrics_data.get(
                "average_block_latency", 0.0
            )

        # Historical performance
        metrics.peak_download_rate = max(
            metrics.peak_download_rate, metrics.download_rate
        )
        metrics.peak_upload_rate = max(metrics.peak_upload_rate, metrics.upload_rate)
        ...
        if metrics.efficiency_score == 0.0 and metrics.connection_duration > 0:
            ...
            if total_bytes > 0:
                efficiency = (total_bytes / metrics.connection_duration) / max(
                    metrics.peak_download_rate + metrics.peak_upload_rate, 1.0
                )
    ...
    def get_global_peer_metrics(self) -> dict[str, Any]:
        ...
        active_peers = sum(
            ...
            if time.time() - metrics.last_activity
            < 300.0  # Active if activity in last 5 minutes
        )
    ...
    def get_system_wide_efficiency(self) -> dict[str, Any]:
        ...
        total_bandwidth_util = sum(
            m.bandwidth_utilization for m in self.peer_metrics.values()
        )
        total_quality = sum(
            m.connection_quality_score for m in self.peer_metrics.values()
        )
        ...
        return {
            "overall_efficiency": total_efficiency / num_peers
            if num_peers > 0
            else 0.0,
            "average_peer_efficiency": total_efficiency / num_peers
            if num_peers > 0
            else 0.0,
            "bandwidth_utilization": total_bandwidth_util / num_peers
            if num_peers > 0
            else 0.0,
            "connection_quality_average": total_quality / num_peers
            if num_peers > 0
            else 0.0,
            "active_connection_ratio": active_peers / num_peers
            if num_peers > 0
            else 0.0,
        }
