...
class TorrentFileExplorerWidget(Container):  # type: ignore[misc]
    ...
    # F2.6.10: bound to App per-torrent reactives via data_bind.
    ...
    def on_mount(self) -> None:  # type: ignore[override]  # pragma: no cover
        ...
        try:
            ...
            try:
                ...
                logger.debug("TorrentFileExplorerWidget data_bind skipped: %s", exc)
    ...
    def _schedule_file_refresh(
        self,
        *,
        files_override: Optional[list[dict[str, Any]]] = None,
        status_override: Optional[dict[str, Any]] = None,
    ) -> None:
        """Schedule async file tree refresh (F2.6.10)."""
        try:
    ...
    def watch_selected_torrent_files(self, value: list[dict[str, Any]]) -> None:  # pragma: no cover
        """Reactive watcher: rebuild tree when bound files change (F2.6.10)."""
        if isinstance(value, list):
            self._schedule_file_refresh(files_override=value)

    def watch_selected_torrent_status(self, value: dict[str, Any]) -> None:  # pragma: no cover
        """Reactive watcher: update base path when bound status changes (F2.6.10)."""
        if isinstance(value, dict) and value:
            self._schedule_file_refresh(status_override=value)
    ...
    async def _refresh_files(
        ...
        *,
        ...
        """Refresh file list from data provider or bound reactives."""
