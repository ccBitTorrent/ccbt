...
class PeersTable(DataTable):  # type: ignore[misc]
    ...
    def on_mount(self) -> None:  # type: ignore[override]
        ...
        self.zebra_stripes = True
        self.add_columns("IP", "Port", "Down (B/s)", "Up (B/s)", "Choked", "Client")
        ...
        try:
            ...
            if getattr(self.alert_manager, "alert_rules", None):
                ...
                rules_renderable = Panel(
                    "No alert rules configured", title="Alert Rules"
                )
            ...
            if getattr(self.alert_manager, "active_alerts", None):
                ...
                act_renderable = Panel("No active alerts", title="Active Alerts")
            ...
            if getattr(self, "alerts", None) is not None:
                self.alerts.update(Panel(alerts_grid, title="Alerts"))
...
def main() -> int:
    ...
    try:
        available = _TEXTUAL_AVAILABLE  # type: ignore[name-defined]
    except Exception:
        available = False
    if not available:
        logger = logging.getLogger(__name__)
    ...
    session = AsyncSessionManager(".")  # type: ignore[call-arg]
    try:
        # TerminalDashboard.on_mount starts the session and metrics, but ensure availability
        run_dashboard(session, refresh=float(args.refresh))
        return 0
    finally:
        ...
        with contextlib.suppress(Exception):
            ...
            if _asyncio.get_event_loop_policy().get_event_loop().is_running():
                # In case we're inside a running loop (rare for console entry), create a task
                task = _asyncio.create_task(session.stop())  # type: ignore[attr-defined]
                task.add_done_callback(lambda _t: None)
            else:
                _asyncio.run(session.stop())  # type: ignore[attr-defined]
