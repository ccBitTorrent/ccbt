...
def parse_magnet(uri: str) -> MagnetInfo:
    ...
    for xt in xts:
        if xt.startswith("urn:btih:"):
            ...
            break
    if not btih_value:
        raise ValueError("Magnet URI missing xt=urn:btih")
    ...
    return MagnetInfo(info_hash=info_hash, display_name=display_name, trackers=trackers, web_seeds=web_seeds)


def build_minimal_torrent_data(info_hash: bytes, name: Optional[str], trackers: List[str]) -> Dict[str, Any]:
...
def build_torrent_data_from_metadata(info_hash: bytes, info_dict: Dict[bytes, Any]) -> Dict[str, Any]:
    ...
    pieces_info = {
        ...
        "total_length": file_info["total_length"] if file_info["type"] == "single" else sum(f["length"] for f in file_info["files"]),
    }
    ...
    return {
        ...
        "name": file_info["name"] if file_info["type"] == "single" else file_info["name"],
    }
