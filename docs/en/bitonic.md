...
The dashboard uses a **tabbed interface** with a split layout:
...
│                                                                         │
│ ┌─────────────────────────────────────────────────────────────────────┐ │
│ │ GRAPHS SECTION (Always visible - Top Half)                         │ │
│ │ ┌───────────────────────────────────────────────────────────────┐ │ │
│ │ │ Global Stats & Graphs                                          │ │ │
│ │ │ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐          │ │ │
│ │ │ │ Download │ │ Upload   │ │ Active   │ │ Peers    │          │ │ │
│ │ │ │ 2.5 MB/s │ │ 1.2 MB/s │ │    3     │ │   25     │          │ │ │
│ │ │ └──────────┘ └──────────┘ └──────────┘ └──────────┘          │ │ │
│ │ │                                                               │ │ │
│ │ │ [Global] [Network] [Disk I/O] [System] [Security] [DHT]      │ │ │
│ │ │                                                               │ │ │
│ │ │ [Speed Graph: ▁▂▃▅▆▇█]                                       │ │ │
│ │ └───────────────────────────────────────────────────────────────┘ │ │
│ └─────────────────────────────────────────────────────────────────────┘ │
│                                                                         │
│ ┌─────────────────────────────────────────────────────────────────────┐ │
│ │ MAIN TABS (Bottom Half)                                             │ │
│ │ ┌───────────────────────────────────────────────────────────────┐ │ │
│ │ │ [Torrents] [Per-Torrent] [Preferences]                        │ │ │
│ │ ├───────────────────────────────────────────────────────────────┤ │ │
│ │ │                                                               │ │ │
│ │ │ Torrents Tab Content:                                         │ │ │
│ │ │ [Global] [Downloading] [Seeding] [Completed] [Active] [Inactive]│ │ │
│ │ │                                                               │ │ │
│ │ │ Info Hash | Name | Size | Progress | Status | Down | Up | ...│ │ │
│ │ │ ───────────────────────────────────────────────────────────── │ │ │
│ │ │ abc123... | file.torrent | 1.2 GB | 45.0% | Active | ...     │ │ │
│ │ │ def456... | movie.torrent | 2.5 GB | 100% | Seeding | ...   │ │ │
│ │ │ ...                                                           │ │ │
│ │ │                                                               │ │ │
│ │ └───────────────────────────────────────────────────────────────┘ │ │
│ └─────────────────────────────────────────────────────────────────────┘ │
...
1. **Torrents Tab** - Lists all torrents with nested sub-tabs:
   - **Global**: All torrents
   - **Downloading**: Active downloads
   - **Seeding**: Completed torrents being seeded
   - **Completed**: Finished downloads
   - **Active**: All active torrents (downloading or seeding)
   - **Inactive**: Paused or stopped torrents

2. **Per-Torrent Tab** - Detailed view for selected torrent:
   - **Torrent Selector**: Dropdown to choose which torrent to view
   - **Files Sub-tab**: File list with selection and priority management
   - **Info Sub-tab**: Detailed torrent information with **Piece Availability Health Bar**
   - **Peers Sub-tab**: Connected peers with ban functionality
...
3. **Preferences Tab** - Configuration with nested sub-tabs:
   ...
   - **Discovery**: DHT, PEX, tracker settings
   - **Security**: Security and IP filtering
   - **Performance**: Performance tuning
   - And more configuration sections...

#### Graphs Section (Always Visible)

The graphs section is always displayed in the top half of the screen and includes:
- **Global Stats Cards**: Download/Upload speeds, Active torrents, Connected peers
- **Graph Sub-tabs**: Access to various monitoring graphs
  - **Global**: Overall statistics
  - **Network**: Network quality metrics
  - **Disk I/O**: Disk I/O statistics
...
- **Availability Percentage**: Percentage of pieces available from at least one peer
- **Piece Count**: Available pieces vs total pieces
- **Average Peers**: Average number of peers per piece
- **Color-Coded Segments**: Thin bars (▌) with colors indicating availability level

Implementation: [ccbt/interface/widgets/piece_availability_bar.py:PieceAvailabilityHealthBar](mdc:ccbt/interface/widgets/piece_availability_bar.py)

### Internationalization (i18n)
The interface supports multiple languages with full i18n integration:
- **Language Selector**: Widget in Preferences → General to change interface language
- **All Strings Wrapped**: All user-facing strings use `_()` translation function
- **Supported Languages**: English, Spanish, French, Japanese, Korean, Hindi, Urdu, Persian, Thai, Chinese, and more
- **Real-time Language Switching**: Changes apply immediately via config update

Implementation: [ccbt/interface/widgets/language_selector.py:LanguageSelectorWidget](mdc:ccbt/interface/widgets/language_selector.py)
...
- **Security Scan** (`x`) - Security statistics, blacklisted IPs, and security events
  - Implementation: [ccbt/interface/screens/monitoring/security_scan.py:SecurityScanScreen](mdc:ccbt/interface/screens/monitoring/security_scan.py)
- **DHT Metrics** (`Ctrl+D`) - DHT routing table statistics and node information
  - Implementation: [ccbt/interface/screens/monitoring/dht_metrics.py:DHTMetricsScreen](mdc:ccbt/interface/screens/monitoring/dht_metrics.py)

See [Monitoring Screens](#monitoring-screens) section for details.

## Dashboard Layout

The dashboard is built with [Textual](https://textual.textualize.io/) and uses a **tabbed interface** with a split layout:

### Layout Structure
- **Header**: Clock and application title
- **Graphs Section** (Top Half - Always Visible): Global statistics and graph sub-tabs
  - Implementation: [ccbt/interface/widgets/core_widgets.py:GraphsSectionContainer](mdc:ccbt/interface/widgets/core_widgets.py)
  - Contains: Summary cards, speed graphs, and access to monitoring screens
- **Main Tabs Section** (Bottom Half): Main tab navigation
  - Implementation: [ccbt/interface/widgets/tabbed_interface.py:MainTabsContainer](mdc:ccbt/interface/widgets/tabbed_interface.py)
  - Contains: Torrents, Per-Torrent, and Preferences tabs
- **Footer**: Status bar and alerts

### Data Access Architecture

**CRITICAL**: The interface uses a strict separation between read and write operations:

- **Read Operations**: All data access goes through `DataProvider`
  - Implementation: [ccbt/interface/data_provider.py](mdc:ccbt/interface/data_provider.py)
  - `DaemonDataProvider`: Uses IPC client for daemon access
  - `LocalDataProvider`: Uses AsyncSessionManager for local access
  - Methods: `get_global_stats()`, `list_torrents()`, `get_torrent_status()`, `get_torrent_peers()`, `get_torrent_files()`, `get_torrent_trackers()`, `get_torrent_piece_availability()`, `get_metrics()`

- **Write Operations**: All commands go through `CommandExecutor`
  ...
  - Methods: `execute_command()` for all torrent/file/config operations

**Never access session internals directly** when using daemon mode. All access must go through `DataProvider` (read) or `CommandExecutor` (write).

### Components

#### Graphs Section Container
Always visible in the top half of the screen. Implementation: [ccbt/interface/widgets/core_widgets.py:GraphsSectionContainer](mdc:ccbt/interface/widgets/core_widgets.py)
- **Summary Cards**: Download/Upload speeds, Active torrents, Connected peers
- **Speed Graphs**: Real-time download/upload speed visualization with sparklines
- **Graph Sub-tabs**: Access to various monitoring graphs (Network, Disk I/O, System, Security, DHT)
- **Real-time Updates**: Subscribes to WebSocket events for live updates

#### Torrents Tab
Main tab for viewing and managing torrents. Implementation: [ccbt/interface/screens/torrents_tab.py:TorrentsTabContent](mdc:ccbt/interface/screens/torrents_tab.py)
- **Nested Sub-tabs**: Filter torrents by state (Global, Downloading, Seeding, Completed, Active, Inactive)
- **Torrents Table**: Shows Info Hash, Name, Size, Progress, Status, Download/Upload speeds, Peers, Seeds
- **Search/Filter**: Filter torrents by name
- **Actions**: Pause, Resume, Remove torrents (via CommandExecutor)

#### Per-Torrent Tab
Detailed view for a selected torrent. Implementation: [ccbt/interface/screens/per_torrent_tab.py:PerTorrentTabContent](mdc:ccbt/interface/screens/per_torrent_tab.py)
- **Torrent Selector**: Dropdown widget to choose which torrent to view
- **Files Sub-tab**: File list with selection, priority, and verification
  - Implementation: [ccbt/interface/screens/per_torrent_files.py:TorrentFilesScreen](mdc:ccbt/interface/screens/per_torrent_files.py)
- **Info Sub-tab**: Detailed torrent information
  - Implementation: [ccbt/interface/screens/per_torrent_info.py:TorrentInfoScreen](mdc:ccbt/interface/screens/per_torrent_info.py)
  - **Piece Availability Health Bar**: Colored progress bar showing piece availability
    - Implementation: [ccbt/interface/widgets/piece_availability_bar.py:PieceAvailabilityHealthBar](mdc:ccbt/interface/widgets/piece_availability_bar.py)
    - Color coding: Green (high), Yellow (medium), Orange (low), Red (very low), Gray (not available)
- **Peers Sub-tab**: Connected peers with ban functionality
  - Implementation: [ccbt/interface/screens/per_torrent_peers.py:TorrentPeersScreen](mdc:ccbt/interface/screens/per_torrent_peers.py)
- **Trackers Sub-tab**: Tracker list with add/remove functionality
  - Implementation: [ccbt/interface/screens/per_torrent_trackers.py:TorrentTrackersScreen](mdc:ccbt/interface/screens/per_torrent_trackers.py)
- **Graphs Sub-tab**: Per-torrent speed graphs
  - Implementation: [ccbt/interface/widgets/graph_widget.py:UploadDownloadGraphWidget](mdc:ccbt/interface/widgets/graph_widget.py)
- **Config Sub-tab**: Per-torrent configuration
  - Implementation: [ccbt/interface/widgets/config_wrapper.py:ConfigScreenWrapper](mdc:ccbt/interface/widgets/config_wrapper.py)

#### Preferences Tab
Configuration management. Implementation: [ccbt/interface/screens/preferences_tab.py:PreferencesTabContent](mdc:ccbt/interface/screens/preferences_tab.py)
- **General Sub-tab**: Language selector and basic settings
  - **Language Selector**: Widget to change interface language
    - Implementation: [ccbt/interface/widgets/language_selector.py:LanguageSelectorWidget](mdc:ccbt/interface/widgets/language_selector.py)
- **Config Sub-tabs**: Various configuration sections (Network, Disk, Discovery, Security, etc.)
  - Implementation: [ccbt/interface/widgets/config_wrapper.py:ConfigScreenWrapper](mdc:ccbt/interface/widgets/config_wrapper.py)
  - Supports dynamic form inputs for editing configuration values
  - Save/Cancel functionality with validation

#### Piece Availability Health Bar
Visual health indicator for torrent piece availability. Implementation: [ccbt/interface/widgets/piece_availability_bar.py:PieceAvailabilityHealthBar](mdc:ccbt/interface/widgets/piece_availability_bar.py)
- **Display**: Thin colored bars (▌) representing each piece's availability
- **Color Coding**:
  - **Green**: High availability (≥70% of max peers)
  - **Yellow**: Medium availability (40-70%)
  - **Orange**: Low availability (20-40%)
  - **Red**: Very low availability (<20%)
  - **Gray**: Not available (0 peers)
- **Summary**: Shows health percentage, available pieces count, and average peers per piece
- **Data Source**: `DataProvider.get_torrent_piece_availability()` (via IPC endpoint `/api/v1/torrents/{info_hash}/piece-availability`)
...
- **TranslationManager**: i18n support with locale management
  - Implementation: [ccbt/i18n/manager.py](mdc:ccbt/i18n/manager.py)

### Screen Architecture

Bitonic follows a hierarchical structure with **Container widgets** for tab content and **Screen classes** for full-screen overlays:

```
TerminalDashboard (Main App)
├── GraphsSectionContainer (Always visible - top half)
│   ├── SummaryCards
│   ├── SpeedSparklines
│   └── Graph sub-tabs (Global, Network, Disk I/O, System, Security, DHT)
└── MainTabsContainer (Bottom half)
    ├── TorrentsTabContent (Container)
    │   └── Nested sub-tabs (Global, Downloading, Seeding, Completed, Active, Inactive)
    ├── PerTorrentTabContent (Container)
    │   ├── TorrentSelector
    │   └── Nested sub-tabs (Files, Info, Peers, Trackers, Graphs, Config)
    │       ├── TorrentFilesScreen (Container)
    │       ├── TorrentInfoScreen (Container) - includes PieceAvailabilityHealthBar
    │       ├── TorrentPeersScreen (Container)
    │       ├── TorrentTrackersScreen (Container)
    │       ├── UploadDownloadGraphWidget
    │       └── ConfigScreenWrapper (Container)
    └── PreferencesTabContent (Container)
        └── Config sub-tabs with ConfigScreenWrapper instances

Full-Screen Overlays (Screen classes):
...
- **Wrapper widgets**: `MonitoringScreenWrapper` and `ConfigScreenWrapper` extract data from Screen classes for embedding in Containers
- **Never embed Screen in Container**: Use wrapper widgets to extract data from screens

**Base Classes**:
- **MonitoringScreen**: Base class for all monitoring screens with common functionality (refresh intervals, navigation, error handling). See [ccbt/interface/screens/base.py:MonitoringScreen](mdc:ccbt/interface/screens/base.py)
- **ConfigScreen**: Base class for configuration screens with unsaved changes detection. See [ccbt/interface/screens/base.py:ConfigScreen](mdc:ccbt/interface/screens/base.py)

**Reusable Widgets**:
- **ProgressBarWidget**: Progress bars for percentages. See [ccbt/interface/widgets/reusable_widgets.py:ProgressBarWidget](mdc:ccbt/interface/widgets/reusable_widgets.py)
- **PieceAvailabilityHealthBar**: Health bar with colored segments for piece availability. See [ccbt/interface/widgets/piece_availability_bar.py:PieceAvailabilityHealthBar](mdc:ccbt/interface/widgets/piece_availability_bar.py)
- **MetricsTableWidget**: Metrics display in table format. See [ccbt/interface/widgets/reusable_widgets.py:MetricsTableWidget](mdc:ccbt/interface/widgets/reusable_widgets.py)
- **SparklineGroup**: Multiple sparklines with labels. See [ccbt/interface/widgets/reusable_widgets.py:SparklineGroup](mdc:ccbt/interface/widgets/reusable_widgets.py)
- **LanguageSelectorWidget**: Language selection widget with flags. See [ccbt/interface/widgets/language_selector.py:LanguageSelectorWidget](mdc:ccbt/interface/widgets/language_selector.py)
- **TorrentSelector**: Dropdown widget for selecting torrents. See [ccbt/interface/widgets/torrent_selector.py:TorrentSelector](mdc:ccbt/interface/widgets/torrent_selector.py)
- **MonitoringScreenWrapper**: Wrapper to embed monitoring screen data in containers. See [ccbt/interface/widgets/monitoring_wrapper.py:MonitoringScreenWrapper](mdc:ccbt/interface/widgets/monitoring_wrapper.py)
- **ConfigScreenWrapper**: Wrapper to embed config screen data in containers. See [ccbt/interface/widgets/config_wrapper.py:ConfigScreenWrapper](mdc:ccbt/interface/widgets/config_wrapper.py)

**Dialogs**:
- **ConfirmationDialog**: Modal dialog for confirmation prompts (e.g., unsaved changes). See [ccbt/interface/screens/base.py:ConfirmationDialog](mdc:ccbt/interface/screens/base.py)
- **FileSelectionDialog**: Modal dialog for selecting files after torrent addition. See [ccbt/interface/screens/dialogs.py:FileSelectionDialog](mdc:ccbt/interface/screens/dialogs.py)
- **PrioritySelectDialog**: Modal dialog for selecting file priority. See [ccbt/interface/screens/per_torrent_files.py:PrioritySelectDialog](mdc:ccbt/interface/screens/per_torrent_files.py)
- **TrackerInputDialog**: Modal dialog for entering tracker URLs. See [ccbt/interface/screens/per_torrent_trackers.py:TrackerInputDialog](mdc:ccbt/interface/screens/per_torrent_trackers.py)

## IPC Endpoints

The interface accesses daemon data through IPC endpoints:

### Torrent Endpoints
- `GET /api/v1/torrents` - List all torrents
- `GET /api/v1/torrents/{info_hash}` - Get torrent status
- `GET /api/v1/torrents/{info_hash}/peers` - Get torrent peers
- `GET /api/v1/torrents/{info_hash}/trackers` - Get torrent trackers
- `GET /api/v1/torrents/{info_hash}/piece-availability` - Get piece availability array
- `POST /api/v1/torrents/{info_hash}/pause` - Pause torrent
- `POST /api/v1/torrents/{info_hash}/resume` - Resume torrent

### Metrics Endpoints
- `GET /api/v1/metrics` - Get Prometheus metrics

All endpoints are accessed through `DataProvider` (read) or `CommandExecutor` (write), never directly via IPC client.
...
User Action
    ↓
TerminalDashboard / Widget
    ↓
DataProvider (read) or CommandExecutor (write)
    ↓
DaemonDataProvider → IPCClient → Daemon IPC Server
    or
LocalDataProvider → AsyncSessionManager
    ↓
