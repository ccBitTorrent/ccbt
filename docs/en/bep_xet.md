...
The XET extension defines the following message types:
...
#### Folder Synchronization Messages

5. **FOLDER_VERSION_REQUEST (0x10)**: Request folder version (git commit reference)
6. **FOLDER_VERSION_RESPONSE (0x11)**: Response with folder version
7. **FOLDER_UPDATE_NOTIFY (0x12)**: Notify peer of folder update
8. **FOLDER_SYNC_MODE_REQUEST (0x13)**: Request synchronization mode
9. **FOLDER_SYNC_MODE_RESPONSE (0x14)**: Response with synchronization mode

#### Metadata Exchange Messages

10. **FOLDER_METADATA_REQUEST (0x20)**: Request folder metadata (.tonic file)
11. **FOLDER_METADATA_RESPONSE (0x21)**: Response with folder metadata piece
12. **FOLDER_METADATA_NOT_FOUND (0x22)**: Metadata not available

#### Bloom Filter Messages

13. **BLOOM_FILTER_REQUEST (0x30)**: Request peer's bloom filter for chunk availability
14. **BLOOM_FILTER_RESPONSE (0x31)**: Response with bloom filter data
...
Offset  Size  Description
0       N     Folder identifier (UTF-8, null-terminated)
N       40    Git commit reference (SHA-1, 20 bytes) or (SHA-256, 32 bytes)
```
...
Offset  Size  Description
0       N     Folder identifier (UTF-8, null-terminated)
N       1     Sync mode (0=DESIGNATED, 1=BEST_EFFORT, 2=BROADCAST, 3=CONSENSUS)
```
...
Offset  Size  Description
0       N     Folder identifier (UTF-8, null-terminated)
N       4     Piece index (big-endian, 0-based)
```
...
Offset  Size  Description
0       N     Folder identifier (UTF-8, null-terminated)
N       4     Piece index (big-endian)
N+4     4     Total pieces (big-endian)
N+8     4     Piece size (big-endian)
N+12    M     Piece data (bencoded .tonic file fragment)
```
...
```
Offset  Size  Description
0       4     Filter size in bytes (big-endian)
```
...
Offset  Size  Description
0       4     Filter size in bytes (big-endian)
4       4     Hash count (big-endian)
8       N     Bloom filter data (bit array)
```
...
1. **DHT (BEP 44)**: Store and retrieve chunk metadata using DHT. Chunk hash (32 bytes) is used as the DHT key. Metadata format: `{"type": "xet_chunk", "available": True, "ed25519_public_key": "...", "ed25519_signature": "..."}`

2. **Trackers**: Announce chunk availability to trackers. First 20 bytes of chunk hash used as info_hash for tracker announces.

3. **Peer Exchange (PEX)**: Extended PEX (BEP 11) with chunk availability messages. `CHUNKS_ADDED` and `CHUNKS_DROPPED` message types exchange chunk hash lists.

4. **Bloom Filters**: Pre-filter chunk availability queries. Peers exchange bloom filters containing their available chunks to reduce network overhead.

5. **Chunk Catalog**: In-memory or persistent index mapping chunk hashes to peer information. Enables fast bulk queries for multiple chunks.

6. **Local Peer Discovery (BEP 14)**: UDP multicast for local network peer discovery. XET-specific multicast address and port configurable.

7. **Multicast Broadcasting**: UDP multicast for chunk announcements on local network.

8. **Gossip Protocol**: Epidemic-style protocol for decentralized update propagation with configurable fanout and interval.

9. **Controlled Flooding**: TTL-based flooding mechanism for urgent updates with priority threshold.

10. **Torrent Metadata**: Extract chunk hashes from torrent XET metadata or BitTorrent v2 piece layers.

**Implementation Notes:**
All 10 discovery mechanisms are integrated into `XetProtocol.announce_torrent()`:
- DHT: Uses `AsyncDHTClient` for chunk discovery (Strategy 1)
- Trackers: HTTP and UDP tracker announces (Strategy 2)
- LPD: Local Peer Discovery via UDP multicast (Strategy 2b)
- PEX: Peer Exchange for chunk availability (Strategy 2c)
- Bloom Filters: Pre-filtering via `XetChunkBloomFilter` (Strategy 3b)
- Catalog: Fast bulk lookups via `XetChunkCatalog` (Strategy 3c)
- Batch/Parallel: Parallel chunk queries via `P2PCASClient` (Strategy 3d)
- Multicast: UDP multicast broadcasting (Strategy 4)
- Gossip: Epidemic-style propagation via `GossipManager` (Strategy 5)
- Flooding: TTL-based controlled flooding (Strategy 6)
- Torrent Metadata: BitTorrent v2 piece layers (Strategy 7)

Each mechanism is called independently with error handling and graceful fallback. See `ccbt/protocols/xet.py` for implementation details.

### Folder Synchronization

XET supports folder synchronization with multiple synchronization modes:

#### Synchronization Modes

- **DESIGNATED (0)**: Single source of truth. One peer designated as source, others sync from it. Automatic source peer election based on uptime and chunk availability.

- **BEST_EFFORT (1)**: All nodes contribute updates, best effort. Conflict resolution via last-write-wins, version-vector, 3-way-merge, or timestamp strategies.

- **BROADCAST (2)**: Specific nodes broadcast updates with queuing. Uses gossip protocol or controlled flooding for propagation.

- **CONSENSUS (3)**: Updates require agreement from majority of nodes. Supports simple majority, Raft consensus, or Byzantine Fault Tolerance (BFT).

**Implementation Notes:**
- Raft consensus is implemented in `ccbt/consensus/raft.py` with state persistence
- Byzantine consensus is implemented in `ccbt/consensus/byzantine.py` with weighted voting support
- Consensus state is persisted to disk and recovered on startup
- Raft leader election, log replication, and safety guarantees are fully implemented
- Byzantine fault tolerance supports configurable fault threshold (default 0.33)
- Consensus callbacks are wired to sync manager's update handler for asynchronous application
- See `ccbt/session/xet_sync_manager.py` for consensus integration

#### Conflict Resolution

When conflicts are detected in BEST_EFFORT mode, the following strategies are available:

- **last-write-wins**: Most recent modification timestamp wins
- **version-vector**: Vector clock-based conflict detection and resolution
- **3-way-merge**: Three-way merge algorithm for automatic conflict resolution
- **timestamp**: Timestamp-based resolution with configurable time windows

#### Git Integration

Folder versions tracked via git commit references (SHA-1 or SHA-256). Changes detected via `git diff`. Auto-commit enabled if `git_auto_commit=True`. Git repository must be initialized in folder root.

**Implementation Notes:**
- Git versioning is integrated into `XetFolder` and `XetSyncManager`
- Current git ref is tracked in sync manager and exchanged during handshake
- Auto-commit functionality triggers on folder changes when enabled
- Git refs are stored in `.tonic` files for version tracking
- See `ccbt/storage/git_versioning.py` for implementation details

#### Allowlist

Encrypted allowlist using Ed25519 for signing and AES-256-GCM for storage. Verified during peer handshake. Aliases supported for human-readable peer names. Allowlist hash exchanged during extension handshake.
...
The `.tonic` file format (similar to `.torrent`) contains XET-specific metadata. The format uses bencoded encoding (same as `.torrent` files) for compatibility.

**Implementation Notes:**
- Tonic files are created and parsed via `TonicFile` class in `ccbt/core/tonic.py`
- Info hash is calculated as SHA-256 of the bencoded `info` dictionary (32 bytes)
- File tree is encoded as nested dictionaries with empty key (`""`) indicating file metadata
- Round-trip testing confirms creation, parsing, and validation work correctly
- All required fields are validated during parsing
- See `ccbt/cli/tonic_generator.py` for tonic file generation from folders
...
dictionary {
    "xet": dictionary {
        ...
        "git_ref": string,           # Git commit reference (SHA-1 or SHA-256)
        "allowlist_hash": string,    # SHA-256 hash of allowlist
        "file_tree": dictionary {    # Nested directory structure
            "path": dictionary {
                "": dictionary {     # Empty key = file metadata
                    "hash": string,   # File hash
                    "size": integer   # File size
                }
            }
        },
        "files": list [              # Flat file list
            dictionary {
                "path": string,
                "hash": string,
                "size": integer
        ...
        "chunk_hashes": list [       # List of chunk hashes (32 bytes each)
            string
        ]
    }
}
...
XET requires UDP port mapping for proper NAT traversal:

- **XET Protocol Port**: Configurable via `xet_port` (defaults to `listen_port_udp`). Mapped via UPnP/NAT-PMP if `map_xet_port=True`.

- **XET Multicast Port**: Configurable via `xet_multicast_port`. Mapped if `map_xet_multicast_port=True` (usually not needed for multicast).

External port information propagated to trackers for proper peer discovery. `NATManager.get_external_port()` supports UDP protocol for XET port queries.
