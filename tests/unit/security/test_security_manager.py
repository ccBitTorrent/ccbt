...
class TestSecurityManager:
    ...
    async def test_cleanup_expired_entries(self, security_manager):
        ...
        security_manager.add_to_blacklist(
            "192.168.1.2", "Test", expires_in=3600, source="test"
        )

        # Cleanup should remove expired entry
        removed = await security_manager.cleanup_expired_entries()

        assert removed == 1
        assert "192.168.1.1" not in security_manager.blacklist_entries
    ...
    async def test_validate_peer_expired_entry(self, security_manager, sample_peer_info):
        ...
        # Validation should remove expired entry and allow connection
        is_valid, reason = await security_manager.validate_peer(sample_peer_info)

        assert is_valid
        assert reason == "Valid peer"
        assert sample_peer_info.ip not in security_manager.blacklist_entries

    def test_add_to_blacklist_with_expiration(self, security_manager):
        """Test adding IP to blacklist with expiration."""
        ip = "192.168.1.100"

        security_manager.add_to_blacklist(ip, "Test reason", expires_in=3600)

        assert ip in security_manager.blacklist_entries
        entry = security_manager.blacklist_entries[ip]
        assert entry.expires_at is not None
        assert entry.source == "manual"

    def test_add_to_blacklist_with_source(self, security_manager):
        """Test adding IP to blacklist with source."""
        ip = "192.168.1.100"

        security_manager.add_to_blacklist(ip, "Test reason", source="auto")
