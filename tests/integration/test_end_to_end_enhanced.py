...
from ccbt.config import init_config, ConfigManager
from ccbt.checkpoint import CheckpointManager
from ccbt.models import TorrentCheckpoint, DownloadStats
...
@pytest.mark.asyncio
async def test_checkpoint_migration_and_resume(tmp_path: Path, session_manager: AsyncSessionManager):
	...
	await cm.save_checkpoint(cp)
	# Migrate JSON -> BINARY and back
	from ccbt.models import CheckpointFormat
	path_bin = await cm.convert_checkpoint_format(info_hash, CheckpointFormat.JSON, CheckpointFormat.BINARY)
	assert path_bin.exists()
	path_json = await cm.convert_checkpoint_format(info_hash, CheckpointFormat.BINARY, CheckpointFormat.JSON)
...
async def test_monitoring_alerts_integration_smoke():
	...
	am.add_alert_rule(AlertRule(name="cpu_test", metric_name="system.cpu", condition="value > 10", severity=AlertSeverity.WARNING, description="cpu test"))
